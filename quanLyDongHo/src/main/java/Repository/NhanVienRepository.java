...
public class NhanVienRepository {
    ...
    public boolean update(NhanVien nv, String maNV) {
        try ( Session session = HibernatUtil.getFACTORY().openSession()) {
            session.getTransaction().begin();
            Query query = session.createQuery("UPDATE NhanVien nv SET nv.ho = :ho, nv.tenDem = :tenDem, nv.ten = :ten, nv.gioiTinh = :gioiTinh, nv.ngaySinh = :ngaySinh, nv.diaChi = :diaChi, nv.sdt = :sdt, nv.email = :email, nv.trangThai = :trangThai, nv.chucVu.id = :chucVu WHERE nv.ma = :ma");
            ...
            query.setParameter("chucVu", nv.getChucVu().getId());
            query.setParameter("ma", maNV);
            query.executeUpdate();
            session.getTransaction().commit();
            return true;
